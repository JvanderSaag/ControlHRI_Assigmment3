...
while run:
    ...
    if port:
        ...
        )/window_scale -fe/np.linalg.norm(B))    ####replace with the valid expression that takes all the forces into account
    ...
    colorMaster = (255,\
         255-np.clip(np.linalg.norm(np.linalg.norm(K)*(xm-xh)/window_scale)*15,0,255),\
         255-np.clip(np.linalg.norm(np.linalg.norm(K)*(xm-xh)/window_scale)*15,0,255)) #if collide else (255, 255, 255)
